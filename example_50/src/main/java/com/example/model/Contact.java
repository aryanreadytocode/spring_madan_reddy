...
@NamedQueries({
        @NamedQuery(name = "Contact.findOpenMsgs", query = "select c from Contact c where c.status = " +
                ":status"),
        @NamedQuery(name = "Contact.updateMsgStatus", query = "Update Contact c set c.status = ?1 " +
                "where c.contactId = ?2")}
)
@NamedNativeQueries({
